...
public class SearchRepository : ISearchRepository
{
    ...
    public async Task<IEnumerable<TaskModel>> SearchAsync(SearchParameterModel model)
    {
        ...
        {
            if (model.Filter.TaskStatus == Enums.TaskStatusFilter.Completed)
            {
                query = query.Where(t => t.IsCompleted);
            }
            else if (model.Filter.TaskStatus == Enums.TaskStatusFilter.Ongoing)
            {
                query = query.Where(t => !t.IsCompleted);
            }
        }
        ...
        {
            Enums.TaskSortOption.CreatedAtAsc => query.OrderBy(t => t.StartDate),
            Enums.TaskSortOption.CreatedAtDesc => query.OrderByDescending(t => t.StartDate),
            Enums.TaskSortOption.DueDateAsc => query.OrderBy(t => t.EndDate),
            Enums.TaskSortOption.DueDateDesc => query.OrderByDescending(t => t.EndDate),
            Enums.TaskSortOption.TitleAsc => query.OrderBy(t => t.Title),
            Enums.TaskSortOption.TitleDesc => query.OrderByDescending(t => t.Title),
            _ => query
        };
